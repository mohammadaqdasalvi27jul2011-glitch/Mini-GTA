#!/usr/bin/env python3
"""
Mini GTA - Mobile App Entry Point
Adapted for mobile/touch interface
"""
...
# Detect platform and set screen size accordingly
import platform
...
if PLATFORM == "Android":
    ...
    SCREEN_HEIGHT = 1920
    IS_MOBILE = True
elif PLATFORM == "Darwin":  # macOS/iOS simulator
    SCREEN_WIDTH = 768
    SCREEN_HEIGHT = 1024
    IS_MOBILE = True
else:
...
class Player(pygame.sprite.Sprite):
    ...
    def handle_input(self, keys, mouse_pos):
        ...
        # Mobile touch input
        if self.touch_pos:
            dx = self.touch_pos[0] - self.x
            dy = self.touch_pos[1] - self.y
            dist = math.sqrt(dx**2 + dy**2)
            if dist > 0:
                self.vx = (dx / dist) * self.speed
                self.vy = (dy / dist) * self.speed
...
class Game:
    def __init__(self):
        print("🎮 Initializing Mini GTA Mobile...")
        self.screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
        pygame.display.set_caption("Mini GTA - Mobile Edition")
        ...
        print("✅ Mini GTA Mobile initialized!")
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.state == GameState.PLAYING:
                    ...
                    self.player.touch_pos = (touch_x + self.camera_x, touch_y + self.camera_y)
                    self.player.shoot(pygame.time.get_ticks(), self.bullets)
            elif event.type == pygame.FINGERUP:
                self.player.touch_pos = None
    ...
    def draw_menu(self):
        ...
        subtitle = font_subtitle.render("Mobile Edition", True, YELLOW)
